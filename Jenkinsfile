pipeline{
    ...
    stages{
        ...
        stage("build-master"){
            ...
            steps{
                sh  'curl -s -X POST https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage?parse_mode=markdown -d chat_id=${TELEGRAM_GROUP_CHAT_ID} -d text="Building *${PROJECT_NAME}* on branch *${BRANCH}*"'
